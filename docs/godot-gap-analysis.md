...
> 2D 排序/tilemap、网络 RPC/房间/反作弊、资源生命周期、打包发布、双脚本后端
> （Lua + QuickJS）、插件系统骨架（编辑器 + 运行时）、NeonPvZ 架构示范工程
> （实体+组件 / prefab 动态生成 / 数据组件 / 波次插件）均已完成。
...
| 脚本 | GDScript 全功能编辑器（断点/单步/变量/调用栈/补全/文档/远程调试） | 双后端：Lua 5.4 + QuickJS (ES2020)，实体脚本/行为树/技能共用同一套引擎绑定、确定性 RNG 与模拟时钟；Lua 带编辑器断点/单步/变量/调用栈；无 JS 调试器、远程调试、悬停文档 | 中 |
| 编辑器 | 完整节点系统 + 检查器 + 场景继承/组 + 动画/地形/tilemap/着色器编辑器 + 导入 watch | ImGui：场景继承、组、节点类型、动画时间线、地形笔刷（实时预览）、tilemap 调色板拖放、UI 可视化编辑器、脚本+调试器、shader 热重载、资产 watch、深色主题 + 图标化工具条 | 中（骨架齐全，打磨与深度差一截） |
| UI | Control 节点树 + 锚点/容器/主题 + 可视化 UI 编辑器 | 自研控件树 + 数据驱动 UI 文档 + ImGui 可视化编辑器（多选/对齐/网格吸附/快捷键）；无锚点/容器布局系统 | 中→小 |
...
| 工程质量 | 成熟（测试/文档/插件生态） | 563 项测试 + 多平台 CI + `--bench` 基准 + 崩溃处理 + 热重载 + 插件系统（编辑器插件/运行时系统模块，Lua+JS） | 中（无社区生态） |
...
2. **编辑器打磨**：多选/批量操作、层级拖拽、地形笔刷实时预览、tilemap 调色板拖放、
   UI 编辑器（多选/对齐/吸附/快捷键）、深色主题与图标化工具条、编辑器插件
   （面板/工具/资产源/组件 schema）已完成；仍缺动画多轨时间线、UI 锚点/容器
   布局系统、检查器曲线编辑等深度工具。
3. **脚本体验**：JS 后端补调试器（断点/单步，复用编辑器调试面板）、Lua 调试器补完整
   调用栈、悬停文档、自动补全作用域感知、远程调试（编辑器与运行中进程通过内存/网络
   通道连接）。
...
2. 编辑器体验迭代（动画多轨时间线 / UI 锚点与容器布局 / 检查器深度工具）
3. 脚本工具链（JS 调试器 / Lua 完整调用栈 / 悬停文档 / 远程调试）
4. 音频效果器/流式 + 2D 光照
