...
- [ ] 传输层：UDP + 重传/ACK、序列号、防作弊随机种子（确定性模拟）。
- [ ] 服务器：无渲染目标（headless）、权威物理/战斗、固定 tick。
- [ ] 状态同步：快照插值/预测回滚、兴趣管理（AOI 九宫格/视野）。
- [ ] 账号/登录/角色选择；分区与副本服务器（world server / instance server）。
