...
| 物理 | 中 | Jolt 同库——**触发器/shape cast/重叠查询/旋转/CCD/关节(固定·铰链·距离)/可配置池+可选多线程已补**；含 `projects/physics_sandbox` 沙盒 demo+回归场景；余：碰撞体类型扩展(胶囊/凸包)、隐式地面消除、约束限位/马达 | A7/A8 + Jolt 封装扩展 |
