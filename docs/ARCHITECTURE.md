...
- **2D 脚本画布**（数据驱动 2D 游戏）：`GameRuntime::Draw` 每帧调用全局 `on_render()`，脚本通过 `DrawRect/DrawRectOutline/DrawText/DrawSprite`（1280x720 设计坐标；`DrawSprite` 经 `loadTexture` 钩子从项目/包加载贴图）把 `Draw2DCmd` 追加进运行时缓冲并刷入渲染器 2D 叠加层；`ReadText(path)` 读取场景/数据文件，`InputMousePos()` 返回设计坐标鼠标位置。示例 `projects/pvz`（植物大战僵尸）：玩法+绘制全在 Lua（向日葵/豌豆/寒冰/坚果/樱桃炸弹、普通/路障/铁桶僵尸、每行推草机、波次表），精灵贴图在 `assets/sprites/`；**关卡布局与 3D 场景统一由场景文件维护**——编辑器 2D 画布读写 `scenes/pvz.json` 的 `level` 字段（9x5 草坪 + 5 植物/3 僵尸笔刷），运行时 `pvz.lua` 也从同一场景文件读取，**F5 / ▶ 试玩直接在编辑器内以 GameRuntime 运行**（`--2d-play` 直达），打包后用 `neon_game` 播放。
