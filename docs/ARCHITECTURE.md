...
- **2D 脚本画布**（数据驱动 2D 游戏）：`GameRuntime::Draw` 每帧调用全局 `on_render()`，脚本通过 `DrawRect/DrawRectOutline/DrawText`（1280x720 设计坐标）把 `Draw2DCmd` 追加进运行时缓冲并刷入渲染器 2D 叠加层；`ReadText(path)` 从项目/包读取关卡等数据文件，`InputMousePos()` 返回设计坐标鼠标位置。示例 `projects/pvz`（植物大战僵尸）：玩法+绘制全在 Lua，编辑器 2D 模式摆放关卡 JSON（9x5 草坪，植物/僵尸笔刷），打包后用 `neon_game` 播放。
