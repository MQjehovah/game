...
- **2D 脚本画布**（数据驱动 2D 游戏）：`GameRuntime::Draw` 每帧调用全局 `on_render()`，脚本通过 `DrawRect/DrawRectOutline/DrawText/DrawSprite`（1280x720 设计坐标；`DrawSprite` 经 `loadTexture` 钩子从项目/包加载贴图）把 `Draw2DCmd` 追加进运行时缓冲并刷入渲染器 2D 叠加层；`ReadText(path)` 读取场景/数据文件，`InputMousePos()` 返回设计坐标鼠标位置。示例 `projects/pvz`（植物大战僵尸）：玩法+绘制全在 Lua（向日葵/豌豆/寒冰/坚果/樱桃炸弹、普通/路障/铁桶僵尸、每行推草机、波次表），精灵贴图在 `assets/sprites/`；**关卡与 3D 场景统一是场景实例**——植物/僵尸是 `scenes/pvz.json` 里带 `plant`/`zombie` 组件的实体（9x5 草坪 + 5 植物/3 僵尸笔刷），编辑器 2D 画布按格子编辑这些实体并写回场景，运行时 `pvz.lua` 从同一场景文件读取（未注册组件按场景数据保留，不进 ECS），**F5 / ▶ 试玩直接在编辑器内以 GameRuntime 运行**（`--2d-play` 直达），打包后用 `neon_game` 播放。
- **数据驱动 3D 游戏**：`projects/neon_realm` 把原硬编码 demo 移植为纯数据项目（112 实体场景 JSON + `scripts/realm.lua`），程序化 mesh key（terrain/tree/house/hero/wolf/npc:r,g,b/rock/water/road…）由引擎生成；脚本用相机相对移动（`cameraYaw` GameVar）、近战/火球/治疗（战斗绑定）、狼群 AI 与波次、NPC 对话、HUD/小地图（`on_render`）、存档（`WriteText/ReadText`）；`SetVisible` 隐藏死怪、`FindNamedEntity` 按名取实体。
